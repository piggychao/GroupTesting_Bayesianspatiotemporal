...
simu_func <- function(seed){
  ...
  Se <- 0.98 #sensitivity for individual testing
  Sp <- 0.99 #specificity for individual testing
  ...
  MH_gibbs <- function(x, y, beta_init, var_beta, spatial_idx,
    ...
    # beta's, tau, rho, random effects, zeta, Se&Sp
    gibbs_samples <- matrix(0, nrow = iter_warmup + iter_sampling, ncol = p+3+effects_len+S)
    ...
    for (g in 1:(iter_warmup + iter_sampling)){
      ...
      ## Sampling S by T random effects - needs to be checked!!!
      ...
      gibbs_samples[g, (p+4):(p+3+effects_len)] <- as.vector(t(effects_init)) #save all random effects
      gibbs_samples[g, (p+4+effects_len):(p+3+effects_len+S)] <- p_pred_mcmc
    }
  ...
  true_par <- c(beta_true,tau_sqrt_tr, rho, zeta_true,  
...
saveRDS(output, file = paste0('VAR_IT1_', time,'.rds'))